...
//
//        $customer_orders_arr = wc_get_orders( array(
...
//        ) );
//
//        echo $customer_orders_count = count($customer_orders_arr);
//        $order_id = '';
//        if($customer_orders_count > 0){
//            $customer_orders = $customer_orders_arr[$customer_orders_count - 1];
//            $order_id = $customer_orders->id;
//        }
//
//        if($order_id != '')
//        {
//            $data = get_user_meta($current_user_id,'user_fitness_custom')[0];
//        }

        $data = get_user_meta($current_user_id,'user_fitness_custom')[0];
		$done_post = get_user_meta($current_user_id, 'is_done_post')[0];
		...
		$limit = get_option('wpfc_limit',3);   // No. of Events
	    $items = array();
		...
		foreach($data as $key=>$value)	{
			...
			$post_date = $value['fitness_post_date'];
			$title = explode(',', $value['fitness_post_name']);//$value['fitness_post_name'];
			$plan_name = strtolower($value['fitness_plan_value']);
			$post_id = $value['fitness_post_id'];//$value['fitness_post_id'][0];
			$post_timestamp = strtotime($post_date);
			$no=count($item_date_counts);
	    	...
	    	if( empty($item_date_counts[$post_date]) || $item_date_counts[$post_date] < $limit ) {
				...
				$post_date_start = date('Y-m-d', $post_timestamp);
				$current_date_start = date('Y-m-d');

				// Display all events till current date
				if(strtotime($post_date_start) <= strtotime($current_date_start)) {
                       ...
                       for ($v = 0; $v < $count; $v++) {
                       	    ...
                       	    $done = 0;
						    foreach ($done_post as $doneVal) {
						        if($doneVal['post_id']== $post_id[$v] && $doneVal['day_num'] == date('dm', $post_timestamp)){
						            $done = 1;
		                        }
		                    }
                       		...
                       		$item = array (
		                        ...
		                        "end" => date('Y-m-d', $post_timestamp),
		                        "url" => site_url().'/index.php/my-account-2/?&profile=post&id='.$post_id[$v].'&cday='.date('dm', $post_timestamp),
		                        "post_id" => $post_id[$v],
		                        "complete" =>$done
					        );

					        			if($item['complete'] == 1) {
                    			$title[$v] =  "Great job today!";
                   			}
		                        ...
		                        "complete" =>$done
									...
									if( strpos( $current_url, $plan_name ) !== false ){ // Event associated with Programs in MENU
										$items[] = apply_filters('wpfc_ajax_post', $item1, $post);
									}
